...
/**
 * Performance test for the Employeur entity.
 */
class EmployeurGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Employeur entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all employeurs")
            .get("/api/employeurs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new employeur")
            .post("/api/employeurs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                ...
                , "maisonMere":"SAMPLE_TEXT"
                , "prenom":"SAMPLE_TEXT"
                , "nom":"SAMPLE_TEXT"
                , "typeIdentifiant":"SAMPLE_TEXT"
                , "numeroIdentifiant":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_employeur_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created employeur")
                .get("${new_employeur_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created employeur")
            .delete("${new_employeur_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
